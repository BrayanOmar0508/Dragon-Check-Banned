...
# === Slash Command: /guilds ===
@bot.slash_command(name="guilds", description="Show all guilds the bot is in")
async def show_guilds(ctx: discord.ApplicationContext):
    guild_names = [f"{i+1}. {guild.name}" for i, guild in enumerate(bot.guilds)]
    guild_list = "\n".join(guild_names)
    await ctx.respond(f"Le bot est dans les guilds suivantes :\n{guild_list}")

# === Slash Command: /lang <en|fr> ===
@bot.slash_command(name="lang", description="Change your preferred language")
async def change_language(ctx: discord.ApplicationContext, lang_code: str):
    lang_code = lang_code.lower()
    if lang_code not in ["en", "fr"]:
        await ctx.respond("❌ Invalid language. Available: `en`, `fr`")
        return
...
# === Slash Command: /check <uid> ===
@bot.slash_command(name="check", description="Check if a UID is banned")
async def check_ban_command(ctx: discord.ApplicationContext, user_id: str):
    ...
    if not user_id.isdigit():
        ...
        await ctx.respond(message[lang])
        return

    await ctx.defer()  # Responds immediately while processing continues

    try:
        ban_status = await check_ban(user_id)
    except Exception as e:
        await ctx.respond(f"{ctx.author.mention} ⚠️ Error:\n```{str(e)}```")
        return
